...
void glictWindow::Paint() {
    ...
    if (this->OnPaint) {
        ...
        r.top = this->y+12;
        r.bottom = this->y+this->height+12;
        r.left = this->x;
        r.right = this->x + this->width;

        c.top = min(this->y+12, this->cliptop);
        c.bottom = min(this->y+this->height+12, this->clipbottom);
        c.left = min(this->x, this->clipleft);
        c.right = min(this->x + this->width, this->clipright);
